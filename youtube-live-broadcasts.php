...
*/
if(!defined('ABSPATH'))exit;$_0=__FILE__;function _1(){register_setting('_2','_3');}add_action('admin_init','_1');
...
__('YouTube Live Broadcasts','_a'),array('description'=>__('Displays the current live broadcast from a specified YouTube channel.',
'_a')));}public function widget($b,$c){echo$b['before_widget'];if(!empty($c['title'])){echo$b['before_title'].
apply_filters('widget_title',esc_html($c['title'])).$b['after_title'];}echo _b();echo$b['after_widget'];}
public function form($c){$d=!empty($c['title'])?$c['title']:__('Live Broadcast','_a');?>
<p><label for="<?php echo $this->get_field_id('title');?>"><?php _e('Title:');?></label>
<input class="widefat" id="<?php echo $this->get_field_id('title');?>" name="<?php echo $this->get_field_name('title');?>" 
type="text" value="<?php echo esc_attr($d);?>"></p><?php }public function update($e,$f){$c=array();
$c['title']=(!empty($e['title']))?strip_tags($e['title']):'';return $c;}}
